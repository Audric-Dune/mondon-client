...
from production.ui.widgets.arret_window.arret_window_ajout_commentaire import ArretWindowAjoutCommentaire
from production.ui.widgets.arret_window.arret_window_ajout_raison import ArretWindowAjoutRaison
from production.ui.widgets.arret_window.arret_window_finish import ArretWindowFinish
from production.ui.widgets.arret_window.arret_window_list_raison import ArretWindowListRaison
from production.ui.widgets.arret_window.arret_window_select_raison import ArretWindowSelectRaison
from production.ui.widgets.arret_window.arret_window_title import ArretWindowTitle
from production.ui.widgets.arret_window.popup_close_avertissement import PopupCloseAvertissement
from production.ui.widgets.arret_window.popup_close_nettoyage import PopupCloseNettoyage
from production.ui.widgets.arret_window.popup_close_no_raison import PopupCloseNoRaison
from production.ui.widgets.arret_window.arret_window_select_type import ArretWindowSelectType
...
class ArretWindow(QMainWindow):
    ...
    def update_widget_from_type(self):
        ...
        # Sinon on regarde si le type d'arret en mémoire correspond ou type d'arret selectionné
            ...
            self.last_type_selected = None
            if self.condition_window_finish():
    ...
    def add_commentaire(self):
        ...
        if commentaire:
            self.arret.add_raison_on_database(commentaire)
            self.arret_window_ajout_commentaire.line_edit.clear()
            self.arret_window_list_raison.update_widget()
            # Utilisation d'un QTimer pour redimensionner la window
            # (on attend que les fonctions ci-dessus soit réellement exécuté)
            QTimer.singleShot(0, self.resize_window)
    ...
    def remove_arret_window_select_raison(self):
        ...
        """
        if self.arret_window_select_raison:
    ...
    def condition_window_finish(self):
        if self.arret.raisons:
            current_type = self.arret.raisons[0].type
            return current_type != "Nettoyage" and current_type != "Commentaire" and not self.arret_window_finish
        else:
            return False
